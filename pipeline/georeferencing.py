...
def _bilinear_interp(col, row, bbox_px, corners):
    """
    Interpole bilineairement la coord monde au pixel (col, row), sachant les
    coords des 4 coins du cadre bbox_px = (x1, y1, x2, y2).
    """
    x1, y1, x2, y2 = bbox_px
    u = (col - x1) / (x2 - x1)
    v = (row - y1) / (y2 - y1)
...
def gcps_from_corners(bbox_px, corners, *, n_samples=5):
    """
    Genere une grille N*N de GCPs sur le cadre, en interpolant les coords
    monde a partir des 4 coins. 5*5 = 25 GCPs par defaut.
    """
    x1, y1, x2, y2 = bbox_px
    gcps = []
    for i in range(n_samples):
...
def gcps_from_grid_intersections(intersections_px, bbox_px, corners,
                                   *, intersections_in_crop_frame=True):
    """
    A partir des intersections du quadrillage detectees par
    pipeline.grid_extraction.grid_intersections, genere un GCP par
    intersection en interpolant ses coords monde depuis les 4 coins.

    intersections_in_crop_frame :
        True  = les intersections sont en pixels DU CROP (typique apres
                preprocess_with_crop). On les translate vers l'image
                originale en ajoutant (x1, y1) du bbox.
        False = les intersections sont deja en pixels de l'image originale.
    """
    x1, y1, _, _ = bbox_px
    gcps = []
    for col, row in intersections_px:
...
def save_gcps_json(gcps, path):
    """Sauvegarde une liste de GCPs en JSON."""
    import json
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = [{"col": g.col, "row": g.row, "x": g.x, "y": g.y, "z": g.z}
