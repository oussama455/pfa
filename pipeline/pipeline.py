...
def run_pipeline(input_path: str | Path,
    ...
    if with_semantic:
        ...
        # device explicite si fourni, sinon auto-detection (GPU sinon CPU)
        if device is None:
            chosen_device = semseg.get_device(verbose=verbose)
        else:
            chosen_device = device
            if verbose:
                print(f"  Device force : {chosen_device}")
        if verbose:
    ...
    # Polygones : eau, végétation, routes rouges (zones)
    poly_layers = ["water", "vegetation", "red_roads"]
    ...
    # Polylignes : courbes de niveau (squelettisées)
